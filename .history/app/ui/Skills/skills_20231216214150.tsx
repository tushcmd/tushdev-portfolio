    ...
    <section className='relative flex flex-col justify-center items-center pt-3'>
      <div className='flex items-start flex-col md:max-w-3xl py-10 w-full md:px-5 px-5 justify-between'>
        ...
        <div className='px-5 mb-20'>
          <ul className='grid grid-cols-2 gap-1 mt-5 md:grid-cols-4 justify-center'>
            {skills.map((skill) => {
              const { title, icon } = skill;
              return (
                <li
                  key={skill.id}
                  className='flex flex-col items-center gap-2 p-5 font-semibold rounded-md text-gray-400 bg-gray-800 opacity-1 transform scale-100 translate-x-0 translate-y-0'
                >
                  {icon}
                  {title}
                </li>
