  ...
  return (
    <section className='relative flex flex-col justify-center items-center md:py-16 py-6'>
      <div className='flex items-start flex-col md:max-w-3xl pb-3 w-full md:px-5 px-5 justify-between'>
        <p className={`{opensans.className} text-3xl text-white`}>{hero.greet}</p>
        <h6
          className={`{opensans.className} text-5xl md:text-8xl text-[#D1D5DB] text-bold pb:6 md:pb-3`}
        >
          {hero.name}
        </h6>
        <p className={`{opensans.className} text-4xl md:text-6xl text-white`}>{hero.paragraph}</p>
      </div>
